  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (list.length > 0) {
        ...
        list.forEach((o) => { o.driver_name = nameMap.get(o.driver_id) ?? "Şoför"; });
      }
      setOffers(list);
      setLoading(false);
    };
    ...
    const channel = supabase
      ...
      .on(
        ...
        async (payload) => {
          ...
          setOffers((prev) => [...prev, { ...row, driver_name: (u as { name?: string })?.name ?? "Şoför" }]);
        }
      )
      .subscribe();
    return () => {
  ...
  const handleSelect = async (offerId: string) => {
    ...
    try {
      const { error: updateOthers } = await supabase
        ...
        .eq("request_id", requestId)
        .neq("id", offerId);
      if (updateOthers) throw updateOthers;

      const { error: updateSel } = await supabase
        .from("offers")
        .update({ status: "accepted" })
        .eq("id", offerId);
      if (updateSel) throw updateSel;
      ...
      <main className="mx-auto flex min-h-screen w-full max-w-[430px] flex-col px-5 py-6">
        ...
        {loading && offers.length === 0 ? (
          <div className="mt-8 flex flex-col items-center gap-4">
            ...
            <p className="text-sm text-[var(--text-dim)]">Şoförler aranıyor...</p>
          </div>
        ) : offers.length === 0 ? (
          ...
          <div className="mt-6 space-y-3">
            {offers.map((o) => (
              <div
                key={o.id}
                className="rounded-[16px] border border-[var(--border)] bg-[var(--bg-card)] p-4 flex items-center justify-between gap-3"
              >
                <div>
                  <p className="font-bold text-[var(--text)]">{o.driver_name ?? "Şoför"}</p>
                  <p className="text-sm text-[var(--text-dim)]">
                    {Number(o.price).toFixed(0)} TL
                    {o.estimated_minutes != null && ` · ~${o.estimated_minutes} dk`}
                  </p>
                </div>
                <button
                  type="button"
                  ...
                  className="rounded-[14px] bg-[#111] px-4 py-2 text-sm font-bold text-white disabled:opacity-50"
                >
                  {selecting === o.id ? "..." : "Seç"}
                </button>
              </div>
